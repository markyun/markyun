...
回答问题再棒，面试官（一般是你顶头上司面你），会考虑，【我要不要这个人做我的同事？】  所以态度一定要好。

非常资深的工程师能把absolute和relative搞混的，这样的人不要也罢，因为团队需要的是你这个人具有可以依靠的才能（靠谱
）。
...
1，iframe会阻塞主页面的Onload事件；2，iframe和主页面共享连接池，而浏览器对相同域的连接有限制，所以会影响页面的并行加载。使用iframe之前需要考虑这两个缺点。
