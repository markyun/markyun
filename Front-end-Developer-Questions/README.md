...
进行追问，可以确保问到你开始不懂或者面试官开始不懂为止，这样可以大大延展题目的区分度和深度，知道你的实际能力。因为这种关联知识是长时期的学习，绝对不是临时记得住的。
...
/*OL  */ CSS规范规定，每个元素都有display属性，确定该元素的类型，每个元素都有默认的display值，比如div默认display属性值为“block”，成为“块级”元素；span默认display属性值为“inline”，是“行内”元素。 


OL   行内元素有：a b span I bem img input select strong（强调的语气）
...
8.html5\CSS3有哪些新特性、移除了那些元素？如何处理HTML5新标签的浏览器兼容问题
【
...
（2）
  ...
  （4）是IE8/IE7/IE6支持通过document.createElement方法产生的标签，可以利用这一特性让这些浏览器支持HTML5新标签，
...
【
 ...
 （1）使用率较高的完备框架有jQuery、YUI、Prototype、Ext.js、Mootools等。尤其是jQuery，超过91%。
